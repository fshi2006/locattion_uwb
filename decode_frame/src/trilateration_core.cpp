...
int GetLocation(vec3d *best_solution, int use4thAnchor, vec3d* anchorArray, int *distanceArray)
{
    ...
    printf("r3=%f\r\n",r3);
 	printf("r4=%f\r\n",r4);
    //std::cout<< "GetLocation" << r1 << r2 << r3 << r4<<std::endl;
    ...
    //std::cout << "GetLocation" << result << "sol1: " << o1.x << o1.y << o1.z << " sol2: " << o2.x << o2.y << o2.z<<std::endl;
...
int main(int argc, char** argv)
{
    ...
    anchorArray[0].z = 1.445; //anchor0.z uint:m

    anchorArray[1].x = 12.370; //anchor1.x uint:m
    anchorArray[1].y = 0.000; //anchor1.y uint:m
    anchorArray[1].z = 1.445; //anchor1.z uint:m

    anchorArray[2].x = 0.000; //anchor2.x uint:m
    anchorArray[2].y = 18.137; //anchor2.y uint:m
    anchorArray[2].z = 1.445; //anchor2.z uint:m

    anchorArray[3].x = 12.370; //anchor3.x uint:m
    anchorArray[3].y = 18.580; //anchor3.y uint:m
    anchorArray[3].z = 1.900; //anchor3.z uint:m
    ...
    while(ros::ok()) // 这里不能用ros::Rate设置延时器,否则会因为延时的原因导致数据解析失败
    {
        if(decoder.ser.available()){
           ...
           decoder.Decode_frame(data[0]);	//进入解帧程序,IMU更新速率不超过300Hz
			Range_deca[0] = (decoder.Range_deca[0]-594.6)*0.9715; //tag to A0 distance
    		Range_deca[1] = (decoder.Range_deca[1]-594.6)*0.9715; //tag to A1 distance
	        Range_deca[2] = (decoder.Range_deca[2]-594.6)*0.9715; //tag to A2 distance
			Range_deca[3] = (decoder.Range_deca[3]-594.6)*0.9715; //tag to A3 distance
		result = GetLocation(&report, 1, &anchorArray[0], &Range_deca[0]);		
   		printf("result = %d\r\n",result);
    	printf("tag.x=%.3f\r\ntag.y=%.3f\r\ntag.z=%.3f\r\n",report.x,report.y,report.z);
		decoder.uwb_data.x = report.x;
		decoder.uwb_data.y = report.x;
		decoder.uwb_data.z = report.x;   
		decoder.pub_uwb.publish(decoder.uwb_data);
        };
        ros::spinOnce();
    }
    return 0;
